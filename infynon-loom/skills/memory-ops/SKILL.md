...
- User asks about Loom or `infynon loom`
- `.infynon/loom/config.toml` is detected in the project
...
3. **Always resolve default user** from `infynon loom` config when `--author` is not specified.
...
infynon loom init --repo <repo-name> --owner <team-name> --user <username>
...
infynon loom note add <id> \
  --title "chrono added" \
  --body "Added for Loom sync timestamps. Low risk." \
  --layer user \
  --scope package \
  --target chrono
```
...
infynon loom retrieve --layer team --scope branch --target <branch-name>

# Notes about a specific file
infynon loom retrieve --scope file --target src/auth.rs
...
# Update note content
infynon loom note update <id> --title "New title" --body "Updated content"
...
infynon loom sync --source team-db --direction both
```
...
infynon loom note update my-note --tags promote,canonical-candidate

# Step 2: After review, create canonical note
infynon loom note add arch-<topic> \
...
infynon loom note update my-note --status archived
```
...
# Remove a backend
infynon loom source remove old-source
...
| Key | Tab | Purpose |
|-----|-----|---------|
| 1 | Overview | Loom status, config summary, layer counts |
...
- Run `infynon loom compact` regularly to clean session and stale notes
