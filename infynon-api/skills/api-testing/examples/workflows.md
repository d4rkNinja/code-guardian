...
infynon api node create --ai "POST /products creates a product with name and price, extracts product_id"
# → creates node id: create-product

infynon api node create --ai "GET /products/{product_id} returns product details"
# → creates node id: get-product

infynon api node create --ai "DELETE /products/{product_id} returns 204"
...
```bash
infynon api flow create "product-lifecycle" --ai "login, create product, get product, then delete it"
...
```bash
infynon api flow run product-lifecycle --base-url http://localhost:3000
```
...
```bash
infynon api ai probe product-lifecycle --base-url http://localhost:3000
```
...
```bash
# Create nodes
infynon api node create --ai "POST /auth/login extracts token and user_role"
infynon api node create --ai "GET /cart/{cart_id} returns cart total and items"
infynon api node create --ai "POST /payment/charge processes payment"
infynon api node create --ai "POST /orders/create creates the order, extracts order_id"

# Build base flow
infynon api flow create "checkout" --ai "login, get cart, create order"

# Add conditional payment branch: only run payment for non-premium users
infynon api attach get-cart payment-charge --condition "body.user_role != premium"
infynon api attach payment-charge create-order
```
...
```bash
# Run the flow
infynon api flow run checkout --base-url http://staging.api.com
...
# Ask AI to explain
infynon api ai explain checkout
```
...
infynon api node create --ai "POST /auth/login extracts token"
infynon api flow create "my-flow" --ai "login flow"

# A week later, add a new endpoint
infynon api node create --ai "POST /notifications/send sends a push notification"

# Ask AI to wire it in
infynon api ai complete my-flow
# AI detects the orphan node and suggests where to connect it

# Or be explicit
infynon api ai attach login-node "the notification endpoint"
```
  ...
  api-tests:
    ...
    steps:
      ...
      - name: Run API flow tests
        run: |
          infynon api flow run auth-flow --base-url http://localhost:3000
          infynon api flow run checkout-flow --base-url http://localhost:3000
          infynon api flow run product-lifecycle --base-url http://localhost:3000

      - name: Run security probes
        run: infynon api ai probe checkout-flow --base-url http://localhost:3000
```
...
```bash
infynon api tui --flow-id checkout-flow
```
...
5. **Press `5`** — Security Probes. See probe results after running `infynon api ai probe`.
